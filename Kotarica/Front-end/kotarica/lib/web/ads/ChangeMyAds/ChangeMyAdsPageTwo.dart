...
class ChangeMyAdsPageTwoWeb extends StatefulWidget {
  final String _title;
  ...
  ChangeMyAdsPageTwoWeb(this._title, this._category, this._subcategory, this.product, this.onProductUpdated);
...
class _State extends State<ChangeMyAdsPageTwoWeb> {
  // controllers
          ...
          leading: IconButton(
            ...
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (context) => ChangeMyAdsPageOneWeb(product:product)
                ),
              );
            },
          ),
    ...
    return GestureDetector(
      onTap: () async {
        if (_advertDescription.text == "" ||
          ...
          await productModel.updateProduct(product.id, title, category, subcategory, _advertDescription.text, rsdPrice.toInt(), ethPrice, data, _izabranaMernaJedinica, _izabranoNudimTrazim, DateTime.now());
          productModel.setProductsReady(false);
          await productModel.getProducts();

          setState(() {
            product = ProductModelWeb.totalProducts[product.id];
