...
class UserModelWeb extends ChangeNotifier {
  ...
  Future<void> initiateSetup([String privateKey]) async {
    ...
    // User aaaa = await UserInfoStore.getUserByUserName("aaaa");
    // print("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA ${aaaa.username}");
    // kondicev deo
  ...
  Future<EthereumAddress> GetOwnerInfo(String username) async {
    ...
    if (answerList[1] == true) {
      korisnik = User(
          ...
          username: answerList[0][0][1], //username
          owner: answerList[0][0][2], //sifra
          json: answerList[0][0][3], //json
          firstName: answerList[0][1][0], //ime
          lastName: answerList[0][1][1], //prezime
          personalAddress: answerList[0][1][2], //adresa
          phone: answerList[0][2][0], //telefon
  ...
  Future<void> updateUserInfo(
    ...
    // List<dynamic> res = await _extendedContract.call(
    //   function: _getUserIndexByUserName,
    //   params: [currentUserName],
    // );
    // print(
    //     "RESULTRESULTRESULTRESULTRESULTRESULTRESULTRESULTRESULTRESULTRESULTRESULTRESULTRESULTRESULTRESULT: $res ${res[0].runtimeType} ${res[1].runtimeType}");
  ...
  Future<void> changeUserOwner({
    ...
    if (newOwner != ownAddress && !await checkUniqueOwner(newOwner)) {
      throw OwnerNotUniqueException2(newOwner);
    }
...
class OwnerNotUniqueException2 implements Exception {
  String cause;
  OwnerNotUniqueException2(EthereumAddress owner) {
