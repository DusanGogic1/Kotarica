...
class SavedAdsModel extends ChangeNotifier {
  int _userId;
  ...
  Future<void> getSavedAds() async {
    ...
    for (var i = 0; i < savedProductsCount; i++) {
      if (savedProducts[i].userId == _userId &&
        ...
        productsToShow.add(ProductModel.getProductById1(savedProducts[i].id));
        productsToShowCount++;
      }
    }
  ...
  Future<void> setPrivateKey() async {
    ...
    print("private key set on startup " + _privateKey + " saved");
  }
